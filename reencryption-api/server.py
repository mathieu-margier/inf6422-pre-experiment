...
@app.route("/api/genkey", methods=["GET"])
def genkey():
...
@app.route("/api/gen_renc_key", methods=["POST"])
...
@app.route("/api/re_encrypt", methods=["POST"])
def re_encrypt():
    if request.content_type.lower() != "application/json":
        abort(415)
...
@app.route("/api/decrypt_reenc", methods=["POST"])
def decrypt_reenc():
    if request.content_type.lower() != "application/json":
        abort(415)
...
@app.route("/api/time_raw/<endpoint>", methods=["GET"])
...
@app.route("/api/time_stats/<endpoint>", methods=["GET"])
